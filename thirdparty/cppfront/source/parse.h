...
struct unqualified_id_node
{
	...
	auto template_arg_id(int i) const
		-> std::string;
...
struct type_id_node
{
	...
	auto is_unqualified_id_node() const
		-> bool
	{
		return (id.index() == unqualified);
	}

	auto get_unqualified_id_node() const
		-> unqualified_id_node*
	{
...
auto unqualified_id_node::template_arg_id(int i) const
-> std::string
{
	auto& t = template_args[i];
	//return std::get<type_id>(t.arg)->to_string();
	if (t.arg.index() == type_id) {
		return std::get<type_id>(t.arg)->to_string();
	}
	if (t.arg.index() == expression) {
		if (std::get<expression>(t.arg)->is_id_expression()) {
			return "?id";
		}
	}
	return "";
...
public:
	...
	auto as_function() const
		-> function_type_node*
	{
		function_type_node* res = std::get<a_function>(type).get();
		return res;
	}
